...
//     let flag = temp.includes(words[i]) // 已经遍历到了
//     if (!flag) {
//       for (let j = 0; j < words.length; j ++) {
//         if (words[j].length < words[i].length || j === i) continue
//         let last = words[j].substr(words[j].length - words[i].length, words[j].length - 1)
//         // if (words[j].includes(words[i])) // 这里错了：word[i]只能是后缀，includes会把不是后缀的word的也舍弃(不是后缀的需要加上)
//         if (last === words[i]) { // word[i]是word[j]的后缀
//           if (words[j] === words[i]) {
//             len = len + words[i].length + 1
//             temp.push(words[i]) // 第一次加入
//           }
//           flag = true // 不加入
//           break
//         }
...
// var minimumLengthEncoding = function(words) {
//   if (words.length === 0) return 0
//   let set = new Set(words) // 去重
//   set.forEach(word => {
//     for(let i = 1; i < word.length; i++) {
//       set.delete(word.substr(i)) // 去除set中有此后缀的单词
//     }
//   })
//   let len = 0
//   set.forEach(word => {
...
//   // 求出节点数 + 叶子节点数
//   let sum = 0
//   function calDepth(children, dep) {
//     let noChild = true
//     // console.log(dep)
//     children.forEach(child => {
//       if (child) {
//         calDepth(child.children, dep + 1)
//         noChild = false
//       }
//     })
//     if (noChild) {
//       sum = sum + dep + 1 // 1：叶子节点
//     }
//   }
//   calDepth(tree.children, 0)
//   return sum
// }
...
var minimumLengthEncoding = function(words) {
  ...
  words = words.map(word => {
    let str = '', i = word.length
    while(--i >= 0) str += word[i]
    return str
  })
  // 排序
  words.sort()
